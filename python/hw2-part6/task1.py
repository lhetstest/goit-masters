...
def total_salary(path: str) -> Tuple[float, float]:
    """
    Аналізує файл із зарплатами розробників і повертає загальну та середню зарплату.

    Аргументи:
        path (str): Шлях до текстового файлу, де кожен рядок містить ім'я розробника та зарплату, розділені комою.

    Повертає:
        Tuple[float, float]: Кортеж із двох чисел:
            - загальна сума зарплат (float),
            - середня зарплата (float, округлена до 2 знаків після коми).

    Обробляє випадки відсутності файлу або некоректних рядків у файлі.
    """
    try:
